...
`start-all.bat` runs Docker Compose profile `infra` (Postgres, Redis, RabbitMQ; Mongo if enabled), then optionally opens app windows from `../repos/<name>` (`runtime.start_apps`). Falls back to hub `scaffolds/` only if repos are missing.
...
`start-all.bat` **does not** start Docker deps; it verifies TCP ports from `project.yaml → infra` and then starts apps from `repos/` if configured.
